// Copyright 2020 Gonçalo Sá <goncalo.sa@consensys.net> Copyright 2016-2019 Federico Bond
// <federicobond@gmail.com> Licensed under the MIT license. See LICENSE file in the project root for
// details.
...
customErrorDefinition: 'error' identifier parameterList ';';

typeDefinition: 'type' identifier 'is' elementaryTypeName ';';
...
functionDefinition:
	functionDescriptor parameterList modifierList returnParameters? (
		';'
		| block
	);
...
parameter:
	typeName stateMutability? storageLocation? identifier?;
...
// In reality catch clauses still are not processed as below the identifier can only be a set
// string: "Error". But plans of the Solidity team include possible expansion so we'll leave this as
// is, befitting with the Solidity docs.
...
inlineAssemblyStatement:
	'assembly' StringLiteralFragment? (
		'(' inlineAssemblyStatementFlag ')'
	)? assemblyBlock;

inlineAssemblyStatementFlag: stringLiteral;
...
revertStatement: 'revert' functionCall ';';
...
Int: 'int' (NumberOfBits)?;

Uint: 'uint' (NumberOfBits)?;

Byte: 'bytes' (NumberOfBytes)?;

Fixed: 'fixed' ( NumberOfBits 'x' [0-9]+)?;

Ufixed: 'ufixed' ( NumberOfBits 'x' [0-9]+)?;

fragment NumberOfBits:
	'8'
	| '16'
	| '24'
	| '32'
...
fragment NumberOfBytes: [1-9] | [12] [0-9] | '3' [0-2];
...
assemblyMember: identifier '.' identifier;
...
// some keywords need to be added here to avoid ambiguities for example, "revert" is a keyword but
// it can also be a function name
identifier: (
		'from'
		| 'calldata'
		| 'receive'
